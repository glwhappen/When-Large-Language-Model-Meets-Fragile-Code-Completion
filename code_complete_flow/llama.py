...
from code_complete import openai_api_complete, openai_api_complete_and_describe, openai_api_complete_function, \
    codegen_api_complate, chatglm_complate, code_llama_complate
...
@cache('./cache/temp/llama_complete')
def flow_code_llama_complete(max_tokens = 200, prompt_max_tokens = 400, lcs_weight = 0.0, led_weight = 1.0, model='chatglm3', temperature=0.7):
    print("获取包含错误的原始代码数据")
    ...
    for i in range(3):
        print("开始进行代码补全")
        huge_code_list = code_llama_complate(huge_code_list, max_tokens=max_tokens, prompt_max_tokens=prompt_max_tokens, model=model, temperature=temperature, random = i)
        print("代码补全完成")
    return
