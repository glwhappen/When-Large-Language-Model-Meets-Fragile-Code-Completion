...
def process_one(key, result, lcs_wight, led_wight):
    ...
    return result[key][0]
@cache("cache/voting/mdoels/get_best_huge_code_list")
...
@cache("cache/voting/models/select_best_complete/{model}")
...
def nearly_wrong():
    ...
    latex_table = styled_df.to_latex(
        # column_format='lcc',
        column_format='l' + 'c' * len(df.columns),
        caption='Same Models with the Different Parameters Nearly Wrong',
        label='Same Models with the Different Parameters Nearly Wrong',
        position_float='centering',
        hrules=True,  # To add horizontal rules, similar to \toprule, \midrule, and \bottomrule in booktabs
    )
    print("latex:")
