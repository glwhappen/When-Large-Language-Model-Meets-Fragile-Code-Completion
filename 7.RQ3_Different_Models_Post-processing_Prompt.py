...
from select_best_line_by_gpt3 import select_best
...
@cache("cache/temp/prompt/temperature/get_best_huge_code_list")
...
@cache("cache/prompt/select_best_complete/{temperature}")
def select_best_complete(lcs_wight = 0.5, led_wight = 0.5, models=["google/gemma-7b"], newpayload={}, temperature=0.7, max_workers=3, max_tokens = 50, prompt_max_tokens = 600):
    ...
    descriptions.append("post-processing prompt")
    ...
    # plot_results(count_result_list)
    # show_result()

    # Re-plotting the DataFrame as a bar chart for each value comparison across the two experiments

    # Transposing the DataFrame for easier plotting of each metric across experiments
    ...
    plt.title(f'prompt {temperature}')
    plt.ylabel('number')
    plt.xlabel('Metrics')
...
def nearly_Correct():
    ...
    for temperature in temperature_list:
        for k in data_list[temperature].keys():
            ...
            data_list[temperature][k] = model_data['补全接近正确']  # model_data['补全接近正确']
            all_data_list[temperature][k] = model_data['补全接近正确'] + model_data['补全接近错误']
    print(data_list)
    ...
    latex_table = styled_df.to_latex(
        ...
        caption='The Correct Results With Post-processing Techniques (Different Models)',
        label='The Correct Results With Post-processing Techniques (Different Models)',
